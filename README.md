...
expose_mode = "prompt"  # prompt | cli_only | opt_in
...
**`expose_mode`** controls how `pre-change` decides visibility for new symbols:

- **`prompt`** (default): Auto-expose Typer commands; prompt to expose other tracked symbols.
- **`cli_only`**: Auto-expose Typer commands and their local package arg types; write `keep_private` for everything else without the expose prompt. Still prompts for a group when a new public CLI module has none.
- **`opt_in`**: Write `keep_private` for every new symbol, including Typer commands. Use `pkg-ext promote` later to make symbols public.

`pre-change --keep-private` overrides config for one run (same as `opt_in`).
