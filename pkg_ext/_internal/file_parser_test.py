...
def _parse_src_module(module) -> PkgSrcFile:
    ...
    pkg_path = next(parent for parent in module_path.parents if parent.name == ASK_SHELL_PKG_IMPORT_NAME)
...
def test_parse_models_module():
    ...
    found_symbols = [(symbol.name, symbol.type) for symbol in sorted(symbols.iterate_ref_symbols())]
    expected_symbols = [
...
def test_create_refs():
    ...
    refs = parse_code_symbols([models_parsed, settings_parsed], ASK_SHELL_PKG_IMPORT_NAME)
    assert refs
