...
from pkg_ext._internal.conftest import CHANGELOG_YAML_FILENAME, E2eDirs, E2eRegressionCheck
from pkg_ext._internal.git_usage.actions import git_commit
from pkg_ext._internal.git_usage.state import GitSince
        ...
        if copy_ignore_globs:
            for glob in copy_ignore_globs:
                for path in list(execution_e2e_dir.rglob(glob)):
                    if path.is_file():
                        path.unlink()
    settings = PkgSettings(repo_root=execution_e2e_dir, pkg_directory=execution_e2e_pkg_path, dev_mode=True)
    if step_number > 1:
...
def _run_command(
    ...
    *,
    ...
    pre_commit_cmd = f"{base_args} pre-commit --git-since {git_since} --skip-docs --skip-dirty-check"
...
def run_e2e(
    ...
    *,
    ...
    if not actual_changelog_path.exists():
        actual_changelog_path = max(path for path in actual_changelog_path.parent.glob("*.yaml"))
    ...
    if skip_regressions:
        return settings
    regression_check(CHANGELOG_YAML_FILENAME, actual_changelog_path.read_text(), extension=".yaml")
...
def _question_patcher(file_expose_answers: dict[str, str], groups: list[str]) -> question_patcher:
    responses = [
        PromptMatch(
